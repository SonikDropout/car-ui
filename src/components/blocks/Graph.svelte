<script>
  ...
  import { CAR_CHARACTERISTICS } from "../../constants";
  ...
  const selectBlocks = {
    fuelCell: {
      title: "Хар-ки БТЭ",
      XOptions: {
        time: { label: "Время", units: "с", icon: "clock" },
...
<div class="layout">
  ...
  <main>
    {#each Object.entries(selectBlocks) as [name, block], i}
      <fieldset>
        ...
        <Select
          ...
          selected={(name == selectedBlock && block.XOptions[selectedX]) || { label: '-- Ось x --' }} />
        <Select
          onChange={selectYOption}
          {name}
          order={i * 2 + 1}
          options={block.YOptions}
          selected={(name == selectedBlock && block.YOptions[selectedY]) || { label: '-- Ось y --' }} />
      </fieldset>
    {/each}
    ...
    <button
      ...
      on:click={saveExcel}
      disabled={!$usbPath}
      title={$usbPath ? 'Записать файл формата xlsx на внешний носитель' : 'Пожалуйста, подключите внешний носитель'}>
